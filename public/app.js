(() => {
  ...
  async function refreshDashboard(){try{const r=await fetch('/api/findings');const d=await r.json();aiContext.findings=d.findings||[];if($('findingCount'))$('findingCount').textContent=aiContext.findings.length;const n=aiContext.findings.filter(x=>['high','critical'].includes(x.severity)).length;if($('dashboardStatus'))$('dashboardStatus').textContent=n?'ATTENTION':'SESSION';if($('securityScore'))$('securityScore').textContent=n?'RISK':'—';}catch{}}
  ...
  async function lookupIP(){const d=$('ipResult'),v=$('ipInput')?.value.trim();if(!d)return;if(!v){d.innerHTML='<div class="empty">Enter an IP address.</div>';return}try{const x=await post('/api/ip',{ip:v});aiContext.ip=x;d.innerHTML='<div class="result-grid">'+Object.entries(x).filter(([k])=>['ip','type','country','region','city','isp','organization','asn','timezone'].includes(k)).map(([k,v])=>`<div class="stat"><small>${esc(k)}</small><b>${esc(v??'—')}</b></div>`).join('')+'</div>'}catch(e){d.innerHTML=`<div class="empty">${esc(e.message)}</div>`}}
  async function checkEmail(){const d=$('emailResult'),v=$('emailInput')?.value.trim();if(!d)return;if(!v){d.innerHTML='<div class="empty">Enter an email address.</div>';return}try{const x=await post('/api/breach',{email:v});aiContext.email=x;d.innerHTML=x.breached?`<div class="score warn">${esc(x.breachCount)} BREACH${x.breachCount===1?'':'ES'}</div>`:'<div class="score good">NO KNOWN BREACHES</div>'}catch(e){d.innerHTML=`<div class="empty">${esc(e.message)}</div>`}}
  ...
  async function askAI(){const i=$('chatInput'),v=i?.value.trim();if(!v)return;const log=$('chatLog');log.insertAdjacentHTML('beforeend',`<div class="bubble user">${esc(v)}</div>`);i.value='';try{const x=await post('/api/ai',{message:`User question:\n${v}\n\nWeb404 context:\n${JSON.stringify(aiContext)}`});log.insertAdjacentHTML('beforeend',`<div class="bubble bot"><b>Web404 AI</b><br>${esc(x.reply).replace(/\n/g,'<br>')}</div>`)}catch(e){log.insertAdjacentHTML('beforeend',`<div class="bubble bot"><b>Web404 AI</b><br>${esc(e.message)}</div>`)}}

  window.lookupDomain=lookupDomain;window.lookupDNS=lookupDomain;window.makeHash=makeHash;window.lookupIP=lookupIP;window.checkEmail=checkEmail;window.usernameScan=usernameScan;window.scanHeaders=scanHeaders;window.loadFindings=loadFindings;window.addFinding=addFinding;window.clearFindings=clearFindings;window.askAI=askAI;window.refreshDashboard=refreshDashboard;
  function bindActions(){
    const map={ipButton:lookupIP,dnsButton:lookupDomain,hashButton:makeHash,headersButton:scanHeaders,emailButton:checkEmail,usernameButton:usernameScan,addFindingButton:addFinding,clearFindingsButton:clearFindings,aiButton:askAI};
    Object.entries(map).forEach(([id,fn])=>{const b=$(id);if(b)b.onclick=e=>{e.preventDefault();fn().catch?.(console.error)}});
    $('chatInput')?.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();$('aiButton')?.click()}});
  }
  function init(){bindNavigation();bindActions();}
