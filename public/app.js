(() => {
  ...
  async function askAI(){const i=$('chatInput'),v=i?.value.trim();if(!v)return;const log=$('chatLog');log.insertAdjacentHTML('beforeend',`<div class="bubble user">${esc(v)}</div>`);i.value='';log.scrollTop=log.scrollHeight;try{const x=await post('/api/ai',{input:`User question:\n${v}\n\nWeb404 context:\n${JSON.stringify(aiContext)}`,context:aiContext});const reply=x.text||'Gemini returned no text response.';log.insertAdjacentHTML('beforeend',`<div class="bubble bot"><b>Web404 AI</b><br>${esc(reply).replace(/\n/g,'<br>')}</div>`)}catch(e){log.insertAdjacentHTML('beforeend',`<div class="bubble bot"><b>Web404 AI</b><br>${esc(e.message)}</div>`)}log.scrollTop=log.scrollHeight}
