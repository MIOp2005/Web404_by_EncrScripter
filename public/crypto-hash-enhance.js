(() => {
  ...
  function renderEncoding() {
    ...
    const convert = (value,format,mode) => { const textEncoder=new TextEncoder(), textDecoder=new TextDecoder(); if(mode==='encode'){if(format==='base64')return toBase64(textEncoder.encode(value));if(format==='hex')return bytesToHex(textEncoder.encode(value));if(format==='url')return encodeURIComponent(value);if(format==='html')return encodeHtml(value);} if(format==='base64')return textDecoder.decode(fromBase64(value));if(format==='hex')return textDecoder.decode(hexToBytes(value));if(format==='url')return decodeURIComponent(value);if(format==='html')return decodeHtml(value);throw Error('Unsupported encoding format.'); };
    const run = mode => { const input=document.getElementById('w404EncodingInput').value,format=document.getElementById('w404EncodingFormat').value,out=document.getElementById('w404EncodingResult'); if(!input){out.textContent='Enter input first.';return;} try { const result=convert(input,format,mode); out.innerHTML='<textarea id="w404EncodingValue" rows="8" readonly></textarea><button type="button" class="crypto-secondary" id="w404CopyEncoding">Copy result</button>'; document.getElementById('w404EncodingValue').value=result; document.getElementById('w404CopyEncoding').onclick=async function(){try{await navigator.clipboard.writeText(result);this.textContent='Copied';setTimeout(()=>this.textContent='Copy result',1200);}catch{this.textContent='Copy failed';setTimeout(()=>this.textContent='Copy result',1200);}}; } catch(error) { out.textContent=error?.message||'Invalid input for the selected format.'; } };
