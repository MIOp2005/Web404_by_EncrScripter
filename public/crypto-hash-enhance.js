(() => {
  ...
  function install(){const nav=document.querySelector('.crypto-tools');if(!nav)return;const old=nav.querySelector('[data-ctool="file"]');if(!old)return;const button=old.cloneNode(true);button.addEventListener('click',e=>{e.stopPropagation();nav.querySelectorAll('.crypto-tool').forEach(x=>x.classList.toggle('active',x===button));render();});old.replaceWith(button);}
