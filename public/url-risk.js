(() => {
  ...
  // High-signal infrastructure patterns. These are indicators, not proof of malware.
  const ipLoggerHosts = new Set([
    'grabify.link','grabify.icu','iplogger.org','iplogger.com','iplogger.co','2no.co',
    'yip.su','iplog.co','ps3cfw.com','blasze.com','spoty.link','iplis.ru'
  ]);
  const shortenerHosts = new Set([
    'bit.ly','tinyurl.com','t.co','is.gd','ow.ly','cutt.ly','rb.gy','shorturl.at','rebrand.ly'
  ]);
  ...
  function hostMatches(host, set) {
    return set.has(host) || [...set].some(x => host.endsWith('.' + x));
  }
  ...
  function assess(target, domain, headers) {
    ...
    const pathQuery = target.pathname + target.search;
    const isIp = /^(?:\d{1,3}\.){3}\d{1,3}$/.test(host) || host.includes(':');
    const params = [...target.searchParams.keys()];

    if (hostMatches(host, ipLoggerHosts)) {
      score += 90; highSignal = true;
      reasons.push('Hostname matches a known IP-logging/tracking service pattern.');
    }
    if (hostMatches(host, shortenerHosts)) {
      score += 18;
      reasons.push('Uses a URL-shortening service, which hides the final destination.');
    }
    ...
    const redirectKeys = params.filter(k => redirectParams.test(k));
    if (redirectKeys.length) { score += 12; reasons.push(`Contains redirect-style parameter${redirectKeys.length > 1 ? 's' : ''}: ${redirectKeys.join(', ')}.`); }
    const trackingKeys = params.filter(k => trackingParams.test(k));
    if (trackingKeys.length >= 2) { score += 10; reasons.push('Contains multiple tracking/IP-related query parameters.'); }
    ...
    if (highSignal || score >= 70) {
      verdict = 'HIGH RISK'; cls = 'high';
      summary = highSignal ? 'A high-signal indicator was detected. Treat this URL as potentially malicious or unsafe.' : 'Multiple strong suspicious indicators were detected.';
    } else if (score >= 35) {
      verdict = 'SUSPICIOUS'; cls = 'medium';
      summary = 'Several suspicious indicators were detected. Verify the destination before opening it.';
    }
    return { score, verdict, cls, summary, reasons, highSignal };
  }

  function privateHosts(host) {
    return /^(localhost|.*\.localhost|.*\.local|0\.0\.0\.0|127(?:\.\d{1,3}){3})$/i.test(host);
  }

  async function analyze() {
    const input = $('urlIntelInput'), output = $('urlIntelResult');
    ...
    catch { output.innerHTML = '<div class="empty">Enter a valid HTTP(S) URL.</div>'; return; }
    ...
    try {
      const r = await fetch('/api/domain', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({domain:target.hostname}) });
      if (r.ok) domain = await r.json();
    } catch {}
    try {
      const r = await fetch('/api/headers', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({url:target.href}) });
      if (r.ok) headers = await r.json();
    } catch {}
    ...
    output.innerHTML = `<div class="url-risk ${risk.cls}">
      <div class="url-risk-top"><div><small>URL RISK VERDICT</small><strong>${risk.verdict}</strong><p>${esc(risk.summary)}</p></div><div class="risk-score"><b>${risk.score}</b><span>/ 100</span></div></div>
      <div class="risk-meter"><span style="width:${risk.score}%"></span></div>
      <div class="url-risk-grid">
        <div class="stat"><small>HOST</small><b>${esc(target.hostname)}</b></div>
        <div class="stat"><small>PROTOCOL</small><b>${esc(target.protocol.replace(':','').toUpperCase())}</b></div>
        <div class="stat"><small>DETECTION CONFIDENCE</small><b>${confidence}</b></div>
        <div class="stat"><small>HTTP SIGNAL</small><b>${headers?.score != null ? `${esc(headers.score)}/100` : 'Unavailable'}</b></div>
      </div>
      <div class="risk-reasons"><h3>Why this verdict?</h3>${reasons}</div>
      <div class="risk-disclaimer"><b>Important:</b> This is a defensive URL-risk engine, not a guaranteed malware/reputation verdict. Known IP-loggers and other high-signal tracking infrastructure are flagged strongly, but a normal-looking URL can still be malicious. Do not enter credentials or download files based on this score alone.</div>
    </div>`;
  }

  function init() {
    const button = $('urlIntelButton');
    if (!button) return;
    button.textContent = 'Check URL Risk →';
    button.onclick = analyze;
    $('urlIntelInput')?.addEventListener('keydown', e => { if (e.key === 'Enter') analyze(); });
  }
