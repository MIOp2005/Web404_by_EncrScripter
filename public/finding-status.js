(() => {
  ...
  let filter = 'all';
  async function load() {
    try { const r = await fetch('/api/findings'); if (!r.ok) return; findings = (await r.json()).findings || []; renderSummary(); renderControls(); renderRiskBreakdown(); } catch {}
  }
  ...
  document.addEventListener('web404:findings-updated',load); if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start(); setInterval(load,10000);
  for (const src of ['/dashboard-v2.js','/ai-v2.js','/report-v2.js']) { const s=document.createElement('script'); s.src=src; document.body.appendChild(s); }
  const reportCss=document.createElement('link'); reportCss.rel='stylesheet'; reportCss.href='/report-v2.css'; reportCss.dataset.web404ReportV2='1'; document.head.appendChild(reportCss);
  const navigationScript=document.createElement('script'); navigationScript.src='/navigation.js'; document.body.appendChild(navigationScript);
})();
