(() => {
  ...
  function renderControls() {
    ...
    [...root.querySelectorAll('.finding-card')].forEach((card, index) => {
      const finding = findings[index]; if (!finding?.id) return;
      card.hidden = !visibleFinding(finding); if (card.querySelector('.finding-status')) return;
      const actions = card.querySelector('.finding-actions') || card;
      const wrap = document.createElement('label'); wrap.className = 'finding-status-wrap'; wrap.append(document.createTextNode('Status'));
      ...
      select.dataset.findingId = finding.id; select.addEventListener('change', () => updateStatus(select)); wrap.appendChild(select); actions.prepend(wrap);
    });
  }
  async function updateStatus(select) {
    const id = select.dataset.findingId, status = select.value; if (!id || !allowed.includes(status) || busy) return;
    ...
    try {
      ...
      if (!r.ok) throw new Error('status update failed'); await load();
  ...
  async function renderRiskBreakdown() {
    ...
    try {
      const r = await fetch('/api/risk', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ findings, observations: {} }) }); if (!r.ok) return;
      const risk = await r.json(), dashboard = $('dashboard'); if (!dashboard) return;
      ...
      const left = document.createElement('div'), b = document.createElement('b'); b.textContent = 'Risk Breakdown'; const s = document.createElement('span'); s.textContent = 'Finding lifecycle impact'; left.append(b, s);
      ...
      [['Findings', breakdown.findings || 0], ['Headers', breakdown.header || 0], ['Email', breakdown.email || 0], ['DNSSEC', breakdown.dnssec || 0]].forEach(([name, value]) => {
        const row = document.createElement('div'); row.className = 'risk-row'; const label = document.createElement('span'); label.textContent = name; const track = document.createElement('i'); const fill = document.createElement('em'); fill.style.width = `${Math.min(100, Math.max(0, Number(value) * 2))}%`; track.appendChild(fill); const val = document.createElement('b'); val.textContent = value; row.append(label, track, val); panel.appendChild(row);
      });
    } catch {}
  }
  ...
  const dashboardScript = document.createElement('script'); dashboardScript.src = '/dashboard-v2.js'; document.body.appendChild(dashboardScript);
  const aiScript = document.createElement('script'); aiScript.src = '/ai-v2.js'; document.body.appendChild(aiScript);
})();