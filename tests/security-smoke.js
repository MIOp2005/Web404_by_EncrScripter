...
assert.match(server, /helmet|Content-Security-Policy|content-security-policy/i, 'Security headers/CSP should be configured');
...
assert.match(server, /MAX_HASH_INPUT\s*=\s*1000000/, 'Hash input cap should be configured');
...
assert.doesNotMatch(server, /passwords|secrets.*return|return.*passwords/i, 'Server should not intentionally return passwords/secrets');
